...
- **Every verification tool is development and CI only, and none may reach production.** veraPDF, qpdf, `pdfsig`, `pdftoppm` and Ghostscript are instruments: nothing in `src/` may invoke one (`tests/ArchTest.php`), and nothing built for testing may ship (`tests/DistributionTest.php` asks `git archive` what a release contains).
